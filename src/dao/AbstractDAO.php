...
abstract class AbstractDAO
{
    ...
    /**
     * Fonction pour securiser une valeur (en cas d'injection SQL)
     * 
     * @param mixed $value valeur à securiser
     * 
     * @return mixed la valeur sécurisée
     */
    ...
    /**
     * Permet de rechercher dans la base de données selon un tri et une limite d'affichage
     * 
     * @param array $orderBy tri de la requete | init => []
     * @param array $limit limite de la requete | init => []
     */
    ...
    /**
     * Genere le code SQL pour un ORDER BY
     * 
     * @param array $orderBy un tableau du tri souhaité
     * 
     * @return string  la chaine de la requete ORDER BY
     */
    ...
    /**
     * Genere le code SQL pour un LIMIT
     * 
     * @param array $limit un tableau de limite souhaité
     * 
     * @return string  la chaine de la requete LIMIT
     */
    ...
    {
        // Enleve la partie reférençant le namespace
        $entityName = explode("\\", $entity::class);
        $entityName = array_slice($entityName, -1); // Recupere le dernier element du tableau
        //var_dump($entityName);

        $getIdMethodEntity = "getId" . $entityName[0];

        var_dump($getIdMethodEntity);
        //exit;

        if ($entity->$getIdMethodEntity()) {
            // update
            $this->update($entity);
        } else {
            // insert
    ...
    protected function insert(EntityInterface $entity): void
    {
        ...
        $setIdMethodEntity = "setId" . $entityName[0];
        ...
        $entity->$setIdMethodEntity($this->pdo->lastInsertId());
    }
    ...
    protected function update(EntityInterface $entity): void
    {
        ...
        $getIdMethodEntity = "getID" . $entity::class;

        $data = $this->getDataFormEntity($columns, $entity);
        $data["id"] = $entity->$getIdMethodEntity();
